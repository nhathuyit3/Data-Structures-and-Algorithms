...
export default class SegmentTree {
   ...
   * @param {function} operation - binary function
  ...
  constructor(inputArray, operation, operationFallback) {
    ...
    // Init array reprentation of segment tree
  ...
  initSegmentTree(inputArray) {
    ...
    if (isPowerOfTwo(inputArrayLength)) {
       ...
       * next number that is a power of two and use it to calculator
  ...
  buildSegmentTree() {
    ...
    const position = 0;
    this.buidTreeRecursively(leftIndex, rightIndex, position);
  }
  /**
   ...
   * @param {number} rightIndex - right index of input aray segment
  ...
  rangeQueryRecursive(queryLeftIndex, queryRightIndex, leftIndex, rightIndex, position) {
    ...
    const rightOperatioResult = this.rangeQueryRecursive(
      queryLeftIndex,
      queryRightIndex,
      middleIndex + 1,
      rightIndex,
    ...
    return this.operation(leftOperationResult, rightOperatioResult);
  }
  /**
   * Left child index
