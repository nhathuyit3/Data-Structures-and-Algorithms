import BinarySearchTree from "../binary-search-tree/BinarySearchTreee";
...
export default class RedBlackTree extends BinarySearchTree {
  ...
  balance(node) {
    ...
    if (node.uncle && this.isNodeRed(node.uncle)) {
      // If node has red uncle then we need to go RECOLORING
      ...
      if (!this.nodeComparator.equal(grandParent, this.root)) {
        ...
        // Since root already has two black sibling that we're just recolored
        return;
      }
      // Now do further checking for recolored grand-parent
      this.balance(grandParent);
    } else if (!node.uncle || this.isNodeBlack(node.uncle)) {
      // If node uncel is black or absent then we need to do ROTATIONS
      if (grandParent) {
